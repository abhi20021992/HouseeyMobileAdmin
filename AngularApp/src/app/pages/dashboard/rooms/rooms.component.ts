...
@Component({
  ...
  template: `
    <nb-card [size]="breakpoint.width >= breakpoints.sm ? 'large' : 'medium'">
      ...
      <houseey-room-selector (select)="select($event)"></houseey-room-selector>
      <houseey-player [collapsed]="isCollapsed() && breakpoint.width <= breakpoints.md"></houseey-player>
    </nb-card>
  `,
})
